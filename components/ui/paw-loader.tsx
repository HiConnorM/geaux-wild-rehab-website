...
/** A single inline paw print drawn with the brand asset. */
function Paw({ px }: { px: number }) {
  return (
    <svg
      width={px}
      height={px}
...
/**
 * Animated paw-print loader for Geaux Wild Rehab.
 * Renders a soft trail of paw prints that step in sequence. No visible text;
 * screen-reader text is provided. Honors prefers-reduced-motion (paws render
 * static when reduced motion is requested).
 */
export function PawLoader({ size = 'md', fullScreen = false, className }: PawLoaderProps) {
  const dims = SIZES[size]
  // Four paws walking in a gentle zig-zag, like little tracks across the area.
  const paws = [0, 1, 2, 3]
  ...
  const trail = (
    <div
      ...
      aria-live="polite"
      className={cn(
        'gw-paw-trail inline-flex items-center',
        !fullScreen && 'text-current',
        className,
      )}
      style={{ gap: dims.gap }}
    >
      {paws.map((i) => (
        <span
          key={i}
          className="gw-paw-step inline-flex"
          style={{
            // Static zig-zag baseline so it reads like footprints on a path.
            transform: `translateY(${i % 2 === 0 ? -dims.rise : dims.rise}px) rotate(${
              i % 2 === 0 ? -dims.tilt : dims.tilt
            }deg)`,
          }}
        >
          <span
  ...
  if (fullScreen) {
    return (
      <div className="gw-paw-fullscreen fixed inset-0 z-50 flex items-center justify-center bg-[var(--brand-offwhite)]/92 backdrop-blur-sm text-[#9aa0c4]">
        {trail}
      </div>
    )
  }
