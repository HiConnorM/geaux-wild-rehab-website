...
export function AnimalsWeHelpSection() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      ctx = gsap.context(() => {
        gsap.set([headerRef.current, ctaRef.current, bannerRef.current], { opacity: 0, y: 32 })
        ...
        gsap.to(ctaRef.current, {
          opacity: 1, y: 0,
          duration: 0.6, ease: EASE_OUT,
          scrollTrigger: { trigger: ctaRef.current, ...ST_DEFAULTS },
        })

        gsap.to(bannerRef.current, {
          opacity: 1, y: 0,
          duration: 0.7, ease: EASE_OUT,
          scrollTrigger: { trigger: bannerRef.current, ...ST_DEFAULTS },
        })
      }, ref)
    })()
  ...
  return (
    <section ref={ref} className="relative z-0 bg-white overflow-hidden -mt-px">
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pt-16 pb-28 md:pt-20 md:pb-36 lg:pt-24 lg:pb-40">
        ...
        {/* Emergency banner */}
        <div ref={bannerRef} className="bg-[#26C9AA] rounded-xl md:rounded-[2rem] p-6 md:p-8 lg:p-10" style={{ opacity: 0 }}>
